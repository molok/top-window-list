...
const SAVED_POSITIONS_KEY = 'molok-panel-positions';
...
class TitleWidget extends St.Widget {
    ...
    constructor() {
        ...
        const hbox = new St.BoxLayout({
            style_class: 'molok-panel-button-box',
            x_expand: true,
            y_expand: true,
        });
        this.add_child(hbox);
        ...
        this._icon = new St.Bin({
            style_class: 'molok-panel-button-icon',
        });
        ...
        this._abstractLabel = new St.Widget({
            style_class: 'molok-panel-button-abstract-label',
            x_expand: true,
            y_expand: true,
        });
        ...
        this._attentionIndicator = new St.Widget({
            style_class: 'molok-panel-button-attention-indicator',
            x_expand: true,
            y_expand: true,
...
class DragActor extends St.Bin {
    ...
    constructor(source, titleActor) {
        super({
            style_class: 'molok-panel-button-drag-actor',
            child: titleActor,
            width: source.width,
        });
...
class BaseButton extends DashItemContainer {
    ...
    constructor(perMonitor, monitorIndex) {
        ...
        this._button = new St.Button({
            style_class: 'molok-panel-button',
            can_focus: true,
            x_expand: true,
...
class MolokPanel extends St.Widget {
    static {
        GObject.registerClass(this);
    }
    ...
    constructor(perMonitor, monitor, extension) {
        ...
        // Vertical layout: spacer (pushes content below the main panel)
        // + horizontal box with window buttons and workspace indicator.
        ...
        this._topSpacer = new St.Widget({
            style: `height: ${Main.panel.height}px;`,
        });
        vbox.add_child(this._topSpacer);
        ...
        this._buttonList = new St.BoxLayout({
            style_class: 'molok-panel-list',
            reactive: true,
            x_align: Clutter.ActorAlign.START,
            x_expand: true,
            y_expand: true,
        });
        box.add_child(this._buttonList);
        ...
        this._workspaceIndicator = new PanelWorkspaceIndicator({
            baseStyleClass: 'molok-panel-workspace-indicator',
            extension,
        });
        ...
        // addChrome() is called before CSS allocation, so the widget has
        // height=0 and the initial strut doesn't reserve any space. Retrack
        // as soon as the actor is first mapped with its real geometry.
        const mappedId = this.connect('notify::mapped', () => {
            if (this.mapped) {
                this.disconnect(mappedId);
                this._retrackChrome(Main.overview.visible
                    ? overviewChromeOptions
                    : chromeOptions);
            }
        });
        ...
        // Keep the top spacer in sync when the main panel height
        // changes (e.g. font scaling).
        this._mainPanelHeightId = Main.panel.connect(
            'notify::height',
    ...
    _updatePosition() {
        // Panel starts at the monitor top edge so the layout manager
        // includes it in the top strut calculation. The internal box
        // has a top margin to push content below the main panel.
    ...
    _getPreferredUngroupedButtonListWidth() {
        if (this._buttonList.get_n_children() === 0)
            return this._buttonList.get_preferred_width(-1)[1];

        const children = this._buttonList.get_children();
        const [, childWidth] = children[0].get_preferred_width(-1);
        const {spacing} = this._buttonList.layout_manager;
    ...
    _checkGrouping() {
        ...
        const maxWidth = this._getMaxButtonListWidth();
        const natWidth = this._getPreferredUngroupedButtonListWidth();
    ...
    _populateButtonList() {
        this._buttonList.destroy_all_children();
    ...
    _updateKeyboardAnchor() {
        Main.layoutManager.keyboardBox.translation_y = 0;
    }
    ...
    _addButton(button, animate) {
        ...
        this._buttonList.add_child(button);
        button.show(animate);
    }
    ...
    handleDragOver(source, _actor, x, _y, _time) {
        ...
        const buttons = this._buttonList.get_children().filter(c => c instanceof BaseButton);
        const buttonPos = buttons.indexOf(source);
        const numButtons = buttons.length;
        const boxWidth = this._buttonList.width;

        // Transform to button list coordinates for index calculation
        // (mostly relevant for RTL to discard workspace indicator etc.)
        x -= this._buttonList.x;
        ...
        if (pos !== this._dragPlaceholderPos && timeDelta >= MIN_DRAG_UPDATE_INTERVAL) {
            ...
            if (sibling)
                this._buttonList.insert_child_below(this._dragPlaceholder, sibling);
            else
                this._buttonList.insert_child_above(this._dragPlaceholder, null);
    ...
    acceptDrop(source, _actor, _x, _y, _time) {
        if (this._dragPlaceholderPos >= 0)
            this._buttonList.set_child_at_index(source, this._dragPlaceholderPos);
    ...
    _savePositions() {
        const buttons = this._buttonList.get_children()
    ...
    _restorePositions() {
        ...
        for (const button of this._buttonList.get_children()) {
            const pos = positions.indexOf(button.id);
            if (pos > -1)
                this._buttonList.set_child_at_index(button, pos);
        }
    }
    ...
    _onItemDragMotion(dragEvent) {
        ...
        const hasTarget = this._buttonList.contains(targetActor);
...
class PanelWorkspaceIndicator extends WorkspaceIndicator {
    static {
        GObject.registerClass(this);
    }
...
export default class MolokPanelExtension extends Extension {
    ...
    enable() {
        ...
        // Load extension stylesheets explicitly — unlike system extensions
        // installed via distro packages, user extensions don't get their
        // CSS loaded by the shell theme automatically.
        const theme = St.ThemeContext.get_for_stage(global.stage).get_theme();
        theme.load_stylesheet(this.dir.get_child('stylesheet-dark.css'));
        theme.load_stylesheet(this.dir.get_child('stylesheet-light.css'));
    ...
    _buildPanels() {
        ...
        Main.layoutManager.monitors.forEach(monitor => {
            if (showOnAllMonitors || monitor === Main.layoutManager.primaryMonitor)
                this._panels.push(new MolokPanel(showOnAllMonitors, monitor, this));
        });
    }
